...
function OrderGroupCard({
  ...
  productGroup: TOrderList;
  index: Number;
}) {
  const { setOrderList, orderList, setSelectedOrderList } = React.useContext(routeContext);
  const removeProduct = () => {
  ...
  return (
    <Link to={`/my-orders/${index}`} className="flex justify-between items-center py-1 bg-slate-100 w-2/3 px-1 rounded-lg mb-2">
      <div className="flex justify-between items-center w-full" onClick={()=>setSelectedOrderList(productGroup.products)}>
        ...
        <XMarkIcon
          className="h-6 w-6 text-black mx-2"
